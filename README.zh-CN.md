...
用户明确指定的模型或推理强度始终优先。每个适用请求都会独立评估：简单任务可以从 Sol 主动降到 Terra 或 Luna，复杂任务也会从 Luna 或 Terra 主动升到 Sol。

## 主要能力

- 生成最小必要的确定性线性 Segment 计划；简单请求仍只用一段，确实复杂或大型的任务可以使用更多阶段。
- 为每段选择模型与推理强度，通过原生同任务覆盖依次切换，最后只恢复一次原路由。
- 每个 Segment 根据自身证据重新选路由，再合并相邻同路由结果。默认预算为 4 段/4 次切换，符合条件时自动扩到 6/6，用户可显式设置到 8/8。
...
- **每次请求重新评估：** 新的简单任务主动降档，新的复杂任务主动升档；当前路由只影响如何切换，不影响模型选择。
