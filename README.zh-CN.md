...
- 自动并发上限为 4，再按有效独立宽度和已观测空闲 worker 减少；总槽位会先扣除主协调线程和已运行 worker。
- 容量未验证时先派发一个 worker，确认空闲后再补位；超过 4 必须证明有足够空闲容量，不预建等待队列。
- 采用关键路径优先的 wait-any 调度降低尾延迟；兼容的短兄弟任务可合并，长任务只在真实独立边界拆分。
- 完整对话留在主线程；worker 只接收目标、必要决策、范围、验收和不可变 ID 组成的上下文胶囊。
...
- 每个 worker 派发确认和结果收到时，都由协调线程用同一 monotonic clock 自动打点；墙钟、累计 worker、峰值并发和利用率全部由逐 worker 区间推导，模型不能填写时间数字。
- 旧的聚合计时记录继续可读，但退出 verified 历史，不再影响并发统计。
- 仅用任务元数据或用户确认统计路由、排队、启动、切换/恢复、有效执行、往返次数和状态门阻塞；缺失值不猜。
- Apply 简报只覆盖当前运行并原样使用运行时生成的并发行；Query/历史明确标注为历史聚合。只有完整的 schema-v2 逐 worker 区间才显示实测指标，否则显示 `测量：待记录`；有效并发倍率只表示工作重叠，不宣称实际提速。
...
Codex 自动路由｜并行计划：3 个任务｜并发：3/4｜可用 worker：3｜来源：smart-reduced｜调度：关键路径优先
并发：峰值 3｜墙钟：120 秒｜累计 worker：288 秒｜有效并发倍率：2.4×｜并发利用率：80%
```
